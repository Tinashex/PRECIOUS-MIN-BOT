...
cmd({
  pattern: "ad",
  alias: ["advertisement", "billboard"],
  react: '📸',
  desc: "Generate an advertisement/billboard style image from your photo.",
  category: "edit",
  filename: __filename
  ...
  try {
    ...
    await reply("⏳ *𝐏𝐑𝐄𝐂𝐈𝐎𝐔𝐒-𝐌𝐃: 𝐆𝐄𝐍𝐄𝐑𝐀𝐓𝐈𝐍𝐆 𝐀𝐃𝐕𝐄𝐑𝐓...*");

    // 2. BUFFER & TEMP STORAGE
    const mediaBuffer = await target.download();
    const extension = mime.includes('png') ? '.png' : '.jpg';
    const tempPath = path.join(os.tmpdir(), `ad_${Date.now()}${extension}`);
    ...
    fs.unlinkSync(tempPath); // Immediate cleanup to save server space
    ...
    if (!response || !response.data) {
      return reply("❌ *𝐄𝐑𝐑𝐎𝐑:* The advertising API is currently unavailable.");
    }
    ...
    await conn.sendMessage(m.chat, {
      image: imageBuffer,
      caption: `📸 *𝐀𝐃𝐕𝐄𝐑𝐓𝐈𝐒𝐄𝐌𝐄𝐍𝐓 𝐆𝐄𝐍𝐄𝐑𝐀𝐓𝐄𝐃*\n\n${FOOTER}`
    }, { quoted: mek });

  } catch (error) {
    console.error("Ad Edit Error:", error);
    reply(`⚠️ *𝐒𝐘𝐒𝐓𝐄𝐌 𝐄𝐑𝐑𝐎𝐑:* Failed to generate ad. Try again later.`);
  }
});
