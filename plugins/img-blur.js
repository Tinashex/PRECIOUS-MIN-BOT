...
cmd({
  pattern: "blur",
  alias: ["bluredit", "soften"],
  react: '📸',
  desc: "Apply a soft blur effect to an image.",
  category: "edit",
  filename: __filename
  ...
  try {
    ...
    await reply("⏳ *𝐏𝐑𝐄𝐂𝐈𝐎𝐔𝐒-𝐌𝐃: 𝐀𝐏𝐏𝐋𝐘𝐈𝐍𝐆 𝐁𝐋𝐔𝐑 𝐄𝐅𝐅𝐄𝐂𝐓...*");

    // 2. BUFFER & TEMP STORAGE
    const mediaBuffer = await target.download();
    const extension = mime.includes('png') ? '.png' : '.jpg';
    const tempPath = path.join(os.tmpdir(), `blur_${Date.now()}${extension}`);
    ...
    fs.unlinkSync(tempPath); // Immediate cleanup to prevent disk bloat
    ...
    if (!response || !response.data) {
      return reply("❌ *𝐄𝐑𝐑𝐎𝐑:* The blur API is currently unavailable.");
    }
    ...
    await conn.sendMessage(m.chat, {
      image: imageBuffer,
      caption: `📸 *𝐁𝐋𝐔𝐑 𝐄𝐅𝐅𝐄𝐂𝐓 𝐀𝐏𝐏𝐋𝐈𝐄𝐃*\n\n${FOOTER}`
    }, { quoted: mek });

  } catch (error) {
    console.error("Blur Edit Error:", error);
    reply(`⚠️ *𝐒𝐘𝐒𝐓𝐄𝐌 𝐄𝐑𝐑𝐎𝐑:* Failed to blur image. Try again later.`);
  }
});
