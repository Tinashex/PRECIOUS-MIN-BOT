...
cmd({
  pattern: "grey",
  alias: ["greyscale", "gray"],
  react: '📸',
  desc: "Convert your image to a classic black and white greyscale effect.",
  category: "edit",
  filename: __filename
  ...
  try {
    ...
    await reply("⏳ *𝐏𝐑𝐄𝐂𝐈𝐎𝐔𝐒-𝐌𝐃: 𝐀𝐏𝐏𝐋𝐘𝐈𝐍𝐆 𝐆𝐑𝐄𝐘𝐒𝐂𝐀𝐋𝐄...*");

    // 2. BUFFER & TEMP STORAGE
    const mediaBuffer = await target.download();
    const extension = mime.includes('png') ? '.png' : '.jpg';
    const tempPath = path.join(os.tmpdir(), `grey_${Date.now()}${extension}`);
    ...
    if (!response || !response.data) {
      return reply("❌ *𝐄𝐑𝐑𝐎𝐑:* The image processing API is currently unavailable.");
    }
    ...
    await conn.sendMessage(m.chat, {
      image: imageBuffer,
      caption: `📸 *𝐆𝐑𝐄𝐘𝐒𝐂𝐀𝐋𝐄 𝐄𝐅𝐅𝐄𝐂𝐓 𝐀𝐏𝐏𝐋𝐈𝐄𝐃*\n\n${FOOTER}`
    }, { quoted: mek });

  } catch (error) {
    console.error("Grey Edit Error:", error);
    reply(`⚠️ *𝐒𝐘𝐒𝐓𝐄𝐌 𝐄𝐑𝐑𝐎𝐑:* Failed to process image. Try again later.`);
  }
});
