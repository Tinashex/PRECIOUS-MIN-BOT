import { proto, downloadContentFromMessage, getContentType, generateWAMessage } from '@whiskeysockets/baileys';
import fs from 'fs';
...
export const sms = (conn, m, store) => {
    ...
    if (m.message) {
        ...
        try {
            m.body = (m.mtype === 'conversation')? m.message.conversation :
                     ...
                     (m.mtype === 'interactiveResponseBody')? JSON.parse(m.message.interactiveResponseBody.bodyText) :
                     (m.message?.interactiveReplyMessage)? (m.message.interactiveReplyMessage.nativeFlowResponseMessage?.paramsJson? JSON.parse(m.message.interactiveReplyMessage.nativeFlowResponseMessage.paramsJson).id : m.message.interactiveReplyMessage.id) :
                     (m.mtype === 'messageContextInfo')? (m.message.buttonsResponseMessage?.selectedButtonId || m.message.listResponseMessage?.singleSelectReply.selectedRowId || m.message.templateButtonReplyMessage?.selectedId || m.text) : '';
        } catch {
            m.body = false
        }
	...
	m.reply = async (content,opt = { packname: "Secktor", author: "SamPandey001" }, type = "text") => {
	  ...
	  let { data, mime } = await conn.getFile(content);
          if (mime == "image/webp") {
            let { writeExifWebp } = await import('../exif.js');
	...
	m.senddoc = (doc,type, id = m.chat, option = { mentions: [m.sender], filename: "owner", mimetype: type }) => conn.sendMessage(id, { document: doc, mimetype: option.mimetype, fileName: option.filename, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
