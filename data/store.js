...
const getGroupMetadata = async (jid) => {
    ...
    const participants = await readJSON(
        `${jid}_participants.json`
    );
...
const saveMessageCount = async (message) => {
    ...
    const messageCounts =
        await readJSON('message_count.json');
...
const getInactiveGroupMembers = async (jid) => {
    ...
    const inactive =
        groupMetadata.participants.filter(participant => {
            const record = messageCounts.find(
                msg =>
                    msg.jid === jid &&
                    msg.sender === participant.id
            );

            return !record || record.count === 0;
        });

    return inactive.map(member => member.id);
};
...
const getChatSummary = async () => {
    const messages =
        await readJSON('message.json');

    const distinctJids =
        [...new Set(messages.map(msg => msg.jid))];
