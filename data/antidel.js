    ...
    try {
        ...
        if (tableInfo.gc_status) {
            ...
            if (oldRecord && oldRecord.length > 0) {
                ...
                const newStatus =
                    oldRecord[0].gc_status ||
                    oldRecord[0].dm_status ||
                    false;

                await DATABASE
                    .getQueryInterface()
                    .dropTable('antidelete');
        ...
        console.error(
            'Error initializing anti-delete settings:',
            error
        );
        ...
        if (
            ...
            error.original.message.includes('no such table')
        ) {
...
async function setAnti(status) {
    ...
    try {
        ...
        console.error(
            'Error setting anti-delete status:',
            error
        );
...
async function getAnti() {
    ...
    try {
        ...
        console.error(
            'Error getting anti-delete status:',
            error
        );
